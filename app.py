...
if page_selection == "➕ Build New Quotation Module":
    ...
    st.caption(f"All base unit cost prices inside the grid should be entered in **USD**. Enter parent row 'No' to create a sub-row.")
    
    conversion_multiplier = exchange_rate if currency_selection == "MMK" else 1.0
    
    # Pre-calculate pricing rollup: Parent price = sum of sub-items
    items_dict = {str(item["No"]): item for item in st.session_state.working_items}
    for item in st.session_state.working_items:
        item["has_children"] = False
        item["child_final_sum"] = 0.0
        item["child_unit_sum"] = 0.0

    for item in st.session_state.working_items:
        p_no = str(item.get("Parent No", "")).strip()
        if p_no and p_no in items_dict:
            items_dict[p_no]["has_children"] = True
            items_dict[p_no]["child_final_sum"] += float(item.get("Final Price", 0.0)) * int(item.get("Qty", 1))
            items_dict[p_no]["child_unit_sum"] += float(item.get("Unit Price", 0.0)) * int(item.get("Qty", 1))

    for item in st.session_state.working_items:
        if item.get("has_children"):
            p_qty = int(item.get("Qty", 1))
            item["Final Price"] = round(item["child_final_sum"] / p_qty, 2) if p_qty > 0 else 0.0
            item["Unit Price"] = round(item["child_unit_sum"] / p_qty, 2) if p_qty > 0 else 0.0

    # Build displaying dataset with clean relational index labels (1.1, 1.2)
    display_list = []
    child_counters = {}
    for item in st.session_state.working_items:
        i_copy = item.copy()
        p_no = str(i_copy.get("Parent No", "")).strip()
        
        if p_no:
            child_counters[p_no] = child_counters.get(p_no, 0) + 1
            i_copy["Display No"] = f"{p_no}.{child_counters[p_no]}"
            i_copy["Description"] = f"   └── {i_copy['Description']}"
        else:
            i_copy["Display No"] = str(i_copy["No"])
            
        base_final_usd = i_copy.get("Final Price", i_copy.get("Unit Price", 0.0))
        i_copy["Display Unit Price"] = round(base_final_usd * conversion_multiplier, 2)
    ...
    edited_items_df = st.data_editor(
        ...
        width="stretch",
        column_config={
            "Display No": st.column_config.TextColumn("No", width="small", disabled=True),
            "No": st.column_config.TextColumn("ID", width="small"),
            "Parent No": st.column_config.TextColumn("Parent No", width="small"),
            ...
            "Final Price": st.column_config.NumberColumn("Final Price", width="collapsed", disabled=True)
        }
    )
    ...
    if not edited_items_df.empty:
        ...
        for idx, r in edited_items_df.iterrows():
            ...
            base_price_usd = r["Unit Price"] / (1 - m_dec) if m_dec < 1 else r["Unit Price"]
            final_prices_usd.append(round(base_price_usd, 2))
            
            desc_clean = str(r["Description"]).replace("   └── ", "").strip()
            clean_descriptions.append(desc_clean)
            
        edited_items_df["Final Price"] = final_prices_usd
        edited_items_df["Description"] = clean_descriptions
        
        save_cols = ["No", "Parent No", "Part Number/Model", "Description", "Qty", "Unit Price", "Margin %", "Final Price"]
        st.session_state.working_items = edited_items_df[save_cols].to_dict(orient="records")
    ...
    # Double counting defense: Sum only main parent rows (where Parent No is empty)
    item_subtotal_usd = 0.0
    if not edited_items_df.empty:
        for idx, r in edited_items_df.iterrows():
            if not str(r.get("Parent No", "")).strip():
                item_subtotal_usd += float(r["Qty"]) * float(r["Final Price"])
    ...
    if action_c2.button("🖨️ Compile Official Corporate PDF Engine Asset"):
        ...
        for idx, r in edited_items_df.iterrows():
            ...
            html_table_rows += f"""
            <tr {row_class}>
                <td style="text-align: center;">{r['Display No']}</td>
                <td style="font-size: 8pt; word-break: break-all !important;">{r['Part Number/Model']}</td>
                <td style="font-size: 8pt; word-break: break-all !important;">{indent_prefix}{r['Description']}</td>
