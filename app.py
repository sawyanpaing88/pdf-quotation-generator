...
if page_selection == "➕ Build New Quotation Module":
    ...
    if "working_items" not in st.session_state or uploaded_doc:
        if items_list:
            ...
            st.session_state.working_items = [
                {"No": "1", "is_sub": False, "parent_idx": "1", "Part Number/Model": "C9300-48TX-E", "Description": "Catalyst 9300 48-port Data Only Network Essentials", "Qty": 1, "Unit Price": 3500.0, "Margin": 20.0},
                {"No": "1.1", "is_sub": True, "parent_idx": "1", "Part Number/Model": "STACK-M-50CM", "Description": "Cisco Catalyst 9300 Stack Cable 50CM", "Qty": 1, "Unit Price": 150.0, "Margin": 10.0}
            ]

    st.subheader("✏️ Live Data Manipulation Grid")
    conversion_multiplier = exchange_rate if currency_selection == "MMK" else 1.0

    # Process and convert prices dynamically across line items
    for item in st.session_state.working_items:
        qty = float(item.get("Qty", 1))
        u_p = float(item.get("Unit Price", 0.0))
        m_pct = float(item.get("Margin", 0.0)) / 100.0
    ...
    # Sync back data tracking alterations back into the structural memory space
    if not edited_df.empty:
        ...
        for idx, row in edited_df.iterrows():
            ...
            updated_records.append({
                ...
                "Part Number/Model": row["Part Number/Model"],
    ...
    with btn_c1:
        if st.button("➕ Add Main Row"):
            # Compute top-level integer sequence counts
            main_rows = [int(float(item["No"])) for item in st.session_state.working_items if not item["is_sub"]]
            ...
            st.session_state.working_items.append({
                ...
                "Qty": 1, "Unit Price": 0.0, "Margin": 0.0
            })
            st.rerun()
    ...
    with btn_c2:
        if st.button("🌿 Add Sub-Row"):
            if st.session_state.working_items:
                # Target the last element current state mapping index configuration
                last_item = st.session_state.working_items[-1]
                p_idx = last_item["parent_idx"]
                
                # Filter down to trace sibling sub-row sequence arrays
                siblings = [item for item in st.session_state.working_items if item["is_sub"] and item["parent_idx"] == p_idx]
    ...
    item_subtotal_base = sum([float(item["Total Price"]) for item in st.session_state.working_items])
    ...
    if action_c2.button("🖨️ Compile Official Corporate PDF Engine Asset"):
        html_table_rows = ""
        for item in st.session_state.working_items:
            row_class = "class='sub-row'" if item["is_sub"] else ""
            indent_prefix = "└── " if item["is_sub"] else ""
            
            qty = float(item["Qty"])
            u_p = float(item["Unit Price"])
            m_pct = float(item["Margin"]) / 100.0
            final_unit_usd = u_p / (1 - m_pct) if m_pct < 1.0 else u_p
            
            converted_unit_price = final_unit_usd * conversion_multiplier
            converted_total_price = item["Total Price"]
            
            html_table_rows += f"""
            <tr {row_class}>
                <td style="text-align: center;">{item['No']}</td>
                <td style="font-size: 8pt; word-break: break-all !important;">{item['Part Number/Model']}</td>
                <td style="font-size: 8pt; word-break: break-all !important;">{indent_prefix}{item['Description']}</td>
